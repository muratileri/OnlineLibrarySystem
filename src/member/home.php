		...
		if(isset($_POST['m_request']))
		{
			...
			else
			{
				...
				if($status == "Checked Out")
					...
					echo error_without_field("This book is waiting for someone else.");
				else if($status == "Available")
				{
					...
					$continue = "pass";
					if($rowLoan >= 3){
						echo error_without_field("You already have 3 reservations.");
					}
					for($i=0; $i<$rowLoan; $i++){
					...
					if($continue == "pass")
					{
						...
						if(!$query->execute())
							echo error_without_field("ERROR: Couldn't request the book");
						else
						{
							...
							$query->execute();
							logActivity('User requested a book.');
